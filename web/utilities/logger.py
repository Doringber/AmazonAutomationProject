...
from amazon.common.configuration import Configuration
from amazon.web.drivers.drivers_ton import DriverClass
...
class Logger(object):
    ...
    def initialization(self, test):
        ...
        self.platform_type = DriverClass
    ...
    def take_screenshot(self, description=None):
        ...
        if self.driver is not None and self.driver.driver is not None:
            ...
            self.driver.take_screenshot(screenshot_file_path)
            self.counter += 1
    ...
    def __setup_logger__(self):
        ...
        # Setup html_logs folder path
        self.test_logs_path = 'html_logs/' + self.test_name

        # Create html_logs path if not exits
        if not path.exists('html_logs'):
            makedirs('html_logs')
